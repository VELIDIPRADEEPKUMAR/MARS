void ENCODER1(){
  ...
  if((GPIOB_BASE->IDR >> 12) & 0B1)  DI1 = 1; // reading PB12 pin for direction 
  else DI1 = -1;
  
  F1 = DI1*(double(1000000/double(T1*N*G)));
...
void ENCODER2(){
  ...
  if((GPIOB_BASE->IDR >> (uint32_t)12) & 0x1)  DI2 = 1; // reading PB12 pin for direction 
  else DI2 = -1;
  
  F2 = DI2*(double(1000000/double(T2*N*G)));       
...
void ENCODER3(){
  ...
  if((GPIOB_BASE->IDR >> (uint32_t)12) & 0x1)  DI3 = 1; // reading PB12 pin for direction 
  else DI3 = -1;
  
  F3 = DI3*(double(1000000/double(T2*N*G)));       
